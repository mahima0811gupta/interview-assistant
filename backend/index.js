...
app.post('/generate_questions', upload.single('file'), async (req, res) => {
  try {
    ...
    const prompt = `You are an expert interviewer. Based on the resume below, generate 4 ${roundType} interview questions. Number them clearly.\n\nResume:\n${resumeText}\n\n${roundType} Round Questions:\n1.`;
...
`;
    ...
    const feedback = response.data?.choices?.[0]?.message?.content || 'No feedback generated.';
    ...
    const candidateFreq = new natural.TfIdf();
    candidateFreq.addDocument(candidateTokens.join(' '));
    candidateFreq.addDocument(questionTokens.join(' '));
